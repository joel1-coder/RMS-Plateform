...
const meetings = [
  { id: 1, scholar: 'Ananya Rao', regNo: 'PH-2021-013', date: 'Oct 24, 2023 - 10:30 AM', type: '1st DC Meeting', status: 'Scheduled', avatar: '#6C63FF' },
  { id: 2, scholar: 'Julian Marsh', regNo: 'PH-2020-078', date: 'Oct 18, 2023 - 02:00 PM', type: 'Pre-Synopsis', status: 'Minutes Pending', avatar: '#10B981' },
  { id: 3, scholar: 'Li Tang', regNo: 'PH-2022-089', date: 'Oct 15, 2023 - 09:00 AM', type: '2nd DC Meeting', status: 'Completed', avatar: '#F59E0B' },
  { id: 4, scholar: 'Sarah Bennett', regNo: 'PH-2022-452', date: 'Nov 02, 2023 - 11:00 AM', type: 'Comprehensive Viva', status: 'Scheduled', avatar: '#EF4444' },
]
    ...
    <div className="animate-fade">
      ...
      <div className="page-body">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '20px' }}>
          <div className="card" style={{ background: 'linear-gradient(135deg,#EEF2FF,#E0E7FF)' }}>
            <div className="card-body" style={{ padding: '16px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                <div style={{ fontSize: '11px', fontWeight: 700, color: '#4338CA', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Upcoming Meetings</div>
                <span style={{ fontSize: '20px' }}>📅</span>
              </div>
              <div style={{ fontSize: '36px', fontWeight: 800, color: '#312E81', lineHeight: 1 }}>{String(upcomingCount).padStart(2,'0')}</div>
              <div style={{ fontSize: '11.5px', color: '#4338CA', marginTop: '4px' }}>✓ Next meeting in 2 days</div>
            </div>
          </div>
          <div className="card" style={{ background: 'linear-gradient(135deg,#FFF7ED,#FEF3C7)' }}>
            <div className="card-body" style={{ padding: '16px 20px' }}>
              ...
              <div style={{ fontSize: '11.5px', color: '#B45309', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <span style={{ background: '#EF4444', color: '#fff', fontSize: '9px', fontWeight: 700, padding: '1px 6px', borderRadius: '99px' }}>3 Action Required</span>
              </div>
            </div>
          </div>
          <div className="card">
            <div className="card-body" style={{ padding: '16px 20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                {['AR','JM','LT'].map((i, idx) => (
                  <div key={idx} style={{ width: 30, height: 30, borderRadius: '50%', background: `hsl(${idx * 80 + 200},60%,55%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '11px', marginLeft: idx > 0 ? '-8px' : 0, border: '2px solid #fff' }}>{i}</div>
                ))}
        ...
        <div className="card">
          <div className="card-header">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <button onClick={handleExport} className="btn btn-ghost btn-sm">⬇️ Export List</button>
              <button
                style={{ padding: '7px 14px', background: 'linear-gradient(90deg,#4F46E5,#6C63FF)', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: '12.5px', fontWeight: 700, cursor: 'pointer' }}
              >
              ...
              <tbody>
                {meetings.map(m => (
                  <tr key={m.id}>
                    ...
                    <td>
                      <span style={{ background: '#EEF2FF', color: '#4338CA', fontSize: '11px', fontWeight: 600, padding: '3px 8px', borderRadius: 'var(--radius-sm)' }}>{m.type}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        ...
                        <button className="btn btn-ghost btn-sm" title="Calendar">📅</button>
                        {m.status === 'Minutes Pending' && (
                          <button style={{ padding: '4px 10px', background: '#EF4444', color: '#fff', border: 'none', borderRadius: 'var(--radius-sm)', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                            Record Minutes
                          </button>
                        )}
                        {m.status === 'Completed' && (
          ...
          <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <button onClick={() => setPage(p => Math.max(1, p-1))} style={{ padding: '6px 12px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: '#fff', cursor: 'pointer', fontSize: '13px' }}>‹</button>
              {[1,2,3].map(p => (
                <button key={p} onClick={() => setPage(p)}
                  style={{ padding: '6px 12px', border: `1px solid ${page === p ? '#4F46E5' : 'var(--border)'}`, borderRadius: 'var(--radius-sm)', background: page === p ? '#4F46E5' : '#fff', color: page === p ? '#fff' : 'var(--text-primary)', cursor: 'pointer', fontWeight: page === p ? 700 : 400, fontSize: '13px' }}>
                  {p}
                </button>
              ))}
              <button onClick={() => setPage(p => Math.min(totalPages, p+1))} style={{ padding: '6px 12px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: '#fff', cursor: 'pointer', fontSize: '13px' }}>›</button>
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginTop: '16px', background: '#F8FAFC' }}>
          <div className="card-body" style={{ display: 'flex', gap: '20px' }}>
            ...
            <div style={{ width: '280px', background: 'linear-gradient(135deg,#1E1B4B,#312E81)', borderRadius: 'var(--radius-md)', padding: '18px', color: '#fff' }}>
