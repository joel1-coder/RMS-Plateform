...
export default function ScholarProfile() {
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'60vh' }}>
      <div className="spinner" style={{ borderColor:'rgba(30,125,69,0.24)', borderTopColor:'#1E7D45' }} />
    </div>
  )
  ...
  return (
    <div className="animate-fade">
        ...
        <div className="topbar-actions" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {user?.isTestAccount && (
            <button 
              ...
              style={{ background: 'linear-gradient(90deg,#1e3a5f,#174EA6)' }} 
          ...
          {editing ? (
            <>
              ...
              <button className="btn btn-primary btn-sm" style={{ background: 'linear-gradient(90deg,#1E7D45,#166A3A)' }} onClick={handleSave}> Save Changes</button>
            </>
          ) : (
            <button className="btn btn-primary btn-sm" style={{ background: 'linear-gradient(90deg,#1E7D45,#166A3A)' }} onClick={() => setEditing(true)}> Edit Profile</button>
          )}
        </div>
      </div>
      ...
      <div className="page-body">
        {user?.isTestAccount && (
          <div style={{
            background: submission?.status === 'Approved' ? '#DEF7EC' : submission?.status === 'Pending' ? '#EBF5FF' : submission?.status === 'Rejected' ? '#FDE8E8' : '#FFF6D8',
            ...
            color: submission?.status === 'Approved' ? '#03543F' : submission?.status === 'Pending' ? '#1E429F' : submission?.status === 'Rejected' ? '#9B1C1C' : '#936C00',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div className="card" style={{ textAlign: 'center', padding: '28px 20px' }}>
              <div style={{
                ...
                background: 'linear-gradient(135deg, #1E7D45, #174EA6)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '34px', fontWeight: 800, color: '#fff',
                boxShadow: '0 8px 24px rgba(30,125,69,0.3)',
              }}>
            ...
            <div className="card card-body">
              ...
              {[
                { icon: '', label: 'Department', value: profile.dept },
            ...
            {/* Supervisor Card */}
            <div className="card card-body" style={{ background: 'linear-gradient(135deg, #E7F4EC, #FFFFFF)' }}>
              <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '12px', color: 'var(--text-primary)' }}>My Supervisor</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <div className="avatar" style={{ background: 'linear-gradient(135deg,#174EA6,#1E7D45)', width: 44, height: 44, fontSize: 16 }}>P</div>
                <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="card">
              ...
              <div className="card-body">
                ...
                {editing && <button className="btn btn-outline btn-sm" style={{ borderColor: '#1E7D45', color: '#1E7D45' }}> Update Password</button>}
              </div>
            </div>
          </div>
        </div>
