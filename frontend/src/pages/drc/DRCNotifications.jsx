...
const TYPE_COLORS = { success: '#10B981', info: '#3B82F6', warning: '#F59E0B', danger: '#EF4444', primary: '#0D9488' }
          ...
          <div style={{ display: 'flex', gap: '4px', padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
            {[{ id: 'all', label: `All (${notifs.length})` }, { id: 'unread', label: `Unread (${unread})` }, { id: 'read', label: 'Read' }].map(t => (
              <button key={t.id} onClick={() => setFilter(t.id)} style={{
                ...
                background: filter === t.id ? '#CCFBF1' : 'transparent',
                color: filter === t.id ? '#0F766E' : 'var(--text-secondary)',
